\documentclass{article}
\usepackage{setspace}
\usepackage{adjustbox,lipsum}
\usepackage{booktabs}
\usepackage{varwidth}
\usepackage{wrapfig}

%% We use A4 paper with slightly smaller margins than the default (scale = 0.7).
\usepackage{geometry}
\geometry{a4paper,hscale=0.75,vscale=0.8}

%% Define the TU Delft house style colors.
\usepackage{color}
\definecolor{tudelft-cyan}{cmyk}{1,0,0,0}
\definecolor{tudelft-black}{cmyk}{0,0,0,1}
\definecolor{tudelft-white}{cmyk}{0,0,0,0}
\definecolor{tudelft-sea-green}{cmyk}{0.54,0,0.32,0}
\definecolor{tudelft-green}{cmyk}{1,0.15,0.4,0}
\definecolor{tudelft-dark-blue}{cmyk}{1,0.66,0,0.4}
\definecolor{tudelft-purple}{cmyk}{0.98,1,0,0.35}
\definecolor{tudelft-turquoise}{cmyk}{0.82,0,0.21,0.08}
\definecolor{tudelft-sky-blue}{cmyk}{0.45,0,0.06,0.06}
\definecolor{tudelft-lavendel}{cmyk}{0.45,0.2,0,0.07}
\definecolor{tudelft-orange}{cmyk}{0.02,0.56,0.84,0}
\definecolor{tudelft-warm-purple}{cmyk}{0.58,1,0,0.02}
\definecolor{tudelft-fuchsia}{cmyk}{0.19,1,0,0.19}
\definecolor{tudelft-bright-green}{cmyk}{0.36,0,1,0}
\definecolor{tudelft-yellow}{cmyk}{0.02,0,0.54,0}

\usepackage{tikz}

\title{Requirements Draft 1} %The title, e.g. Algoritmiek Assignment 1
\author{
  Hambenne, Arnaud\\
  \and
  Jahanshahi, Soheil\\
}
\begin{document}
\maketitle

%% Insert the TU Delft logo at the bottom of the page.
\begin{tikzpicture}[remember picture,overlay]
    \node at (current page.north east)[anchor=north east,inner sep=0pt]{
        \includegraphics{img/logo.pdf}
    };
\end{tikzpicture}

\section{The Application}
\input{./Sections/application.tex}

\subsection{Users}
\input{./Sections/users.tex}

\subsection{Uploading}
\input{./Sections/uploading.tex}

\subsection{Versioning}
\input{./Sections/versioning.tex}

\subsection{Logging}
\input{./Sections/logging.tex}

\section{Scheduling}
\input{./Sections/scheduling.tex}

\subsection{By template}
\input{./Sections/schedulingbytemplate.tex}

\subsection{By section}
\input{./Sections/schedulingbysection.tex}

\section{Suggestions}
\input{./Sections/suggestions.tex}

\subsection{By section}
\input{./Sections/suggestionsbysection.tex}

\subsection{Done / Discarded}
\input{./Sections/suggestionsdonediscarded.tex}

\subsection{By chat}
\input{./Sections/suggestionsbychat.tex}

\section{Feedback}
\input{./Sections/feedback.tex}
	
\end{document}