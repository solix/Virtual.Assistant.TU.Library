package plugins.com.feth.play.module.pa.providers.password;
...
public abstract class UsernamePasswordAuthProvider<R, UL extends UsernamePasswordAuthUser, US extends UsernamePasswordAuthUser, L extends UsernamePasswordAuthProvider.UsernamePassword, S extends UsernamePasswordAuthProvider.UsernamePassword>
	...
	public Object authenticate(final Context context, final Object payload)
		...
		if (payload == Case.SIGNUP) {
			...
			case WRONG_PASSWORD:
				...
				// knows they signed up for our plugins.service
			case NOT_FOUND:
	 ...
	 * customize mail sending e.g. by using a different mailer plugins.service
	 * implementation.
	 * 
	 * @param subject
	 *            The mail's subject.
	 ...
	 *         action.
	 */
	protected Cancellable sendMail(final String subject, final Mailer.Mail.Body body,
			final String recipient) {
		return sendMail(new Mailer.Mail(subject, body, new String[] { recipient }));
	}
	 ...
	 *         action.
	 */
	protected Cancellable sendMail(final Mailer.Mail mail) {
	...
	protected abstract Mailer.Mail.Body getVerifyEmailMailingBody(
